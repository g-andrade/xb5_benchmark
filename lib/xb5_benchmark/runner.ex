defmodule Xb5Benchmark.Runner do
  ...
  defmodule Collector do
    typedstruct do
      ...
      field(:results_per_n, %{non_neg_integer => Xb5Benchmark.Runner.ResultsForSize.t, enforce: true}, enforce: true)
    end
  end
  ...
  @type sample :: nonempty_improper_list(
    count :: pos_integer(), 
  ...
  defp new_collectors(cases) do
    ...
    |> Map.new(
      fn {collector_key, [%Case{} = example_case | _] = cases} ->
        results_per_n = 
          Map.new(
            cases,
            fn %Case{} = c ->
  ...
  defp assign_sampling_group_numbers(number, [c = %Case{} | next], collectors, mapped_cases, sampling_numbers_map) do
    ...
    assign_sampling_group_numbers(number + 1, next, collectors, mapped_cases, sampling_numbers_map)
  end
  ...
  defp run_list_amount_of_reshuffles(cases, time_to_run_cases) do
    max_reshuffles_that_can_run = div(System.convert_time_unit(@batch_interval_seconds, :second, :native), time_to_run_cases)
  ...
  defp run_batches(%State{} = state) do
    ...
    progress_str = "#{convergence_progress}% [#{state.convergence_count}/#{state.convergence_limit}]"
    Logger.notice("Running batch ##{state.batch_nr} for #{batch_interval_seconds}s (#{progress_str})")

    batch_samples = run_recur(state.run_list, state.run_list, state.min_measurement_interval, stop_ts, [])
  ...
  defp collect_samples({collector_key, collector_samples}, collectors) do
    ...
    merged_results_per_n = Enum.reduce(collector_samples, collector.results_per_n, &merge_results_for_size/2)
  ...
  defp processed_samples([sample | next], acc) do
    ...
    processed_sample = (System.convert_time_unit(1, :second, :native) * count) / measurement_duration
